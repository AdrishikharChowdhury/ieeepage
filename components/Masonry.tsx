...
const Masonry: React.FC<MasonryProps> = ({
  ...
  useEffect(() => {
    preloadImages(items.map((i) => i.src)).then(() => setImagesReady(true));
  }, [items]);
  ...
  useEffect(() => {
    ...
    setGridHeight(Math.max(...colHeights) - gap);
  ...
  return (
    <div ref={containerRef} className="relative w-full" style={{ height: gridHeight || 'auto' }}>
      {grid.map((item) => (
        <div
          key={item.id}
          data-key={item.id}
